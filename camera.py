...
while(True):
    ...
    msg = ""
    scanner = pyzbar.decode(frame)
    if len(scanner) > 0 and lastDetected > 50:
        data = scanner[0].data.decode('utf-8')
        lastDetected = 0
        value = int(format(data))
        ...
        cv2.putText(recognizedFrame, msg, bottomLeftCornerOfText, font, fontScale,fontColor,thickness,lineType)
